...
const char* CALLMEBOT_PHONE_NUMBER = "+9779866445340";  // REPLACE with your number (e.g., +11234567890)
const char* CALLMEBOT_API_KEY = "4873404";              // REPLACE with your CallMeBot WhatsApp API Key
    ...
    if (!isnan(diff) && !isnan(last_diff)) {
      ...
      if (crossed && diff < kEdgeThreshold) {
        ...
        if (last_heartbeat == 0) {
          ...
          float r = ((stat_red.maximum() - stat_red.minimum()) / stat_red.average()) / ((stat_ir.maximum() - stat_ir.minimum()) / stat_ir.average());
...
void updateDisplay() {
  ...
  Serial.print(F("Display printh HR: "));
  Serial.print(lastBpm);
  Serial.print(F(" BPM | O2: "));
  Serial.print((int)lastSpo2);
  Serial.print(F("% | T: "));
